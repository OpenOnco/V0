...
import { query, transaction, getClient, close } from '../db/client.js';
...
/**
 * Add item directly to guidance_items table
 * @param {Object} article - Article data
 * @param {Object} classification - AI classification
 * @param {number} runId - Crawler run ID
 */
async function addToGuidanceItems(article, classification, runId) {
  ...
  const insertResult = await query(
    `INSERT INTO mrd_guidance_items (
       ...
       full_text_excerpt, decision_context, extraction_version
    ...
    [
      'pubmed',
      article.pmid,
      article.sourceUrl || `https://pubmed.ncbi.nlm.nih.gov/${article.pmid}/`,
      article.title,
      classification.summary || article.abstract?.substring(0, 500),
      classification.evidence_type || 'review',
      ...
      JSON.stringify({
        ...
        tests_mentioned: classification.tests_mentioned,
        is_practice_changing: classification.is_practice_changing,
  ...
  for (const setting of classification.clinical_settings || []) {
    await query(
...
export async function runPubMedCrawler(options = {}) {
  ...
  try {
    ...
    if (!dryRun && classified.length > 0) {
      logger.info('Step 5: Adding to guidance items');
      ...
      for (const article of classified) {
        try {
          ...
          // Add directly to guidance items
          const id = await addToGuidanceItems(
            article,
