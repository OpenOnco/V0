...
const TestDetailModal = ({ test, category, onClose }) => {
  ...
  // Data completeness — count filled fields from comparisonParams
  const categoryFields = comparisonParams[category] || [];
  const hasValue = (val) => val != null && String(val).trim() !== '' && val !== 'N/A' && val !== 'Not disclosed';
  const filledFieldCount = categoryFields.filter(p => hasValue(test[p.key])).length;
  const completenessPercent = categoryFields.length > 0 ? Math.round((filledFieldCount / categoryFields.length) * 100) : 0;
    ...
    <TestContext.Provider value={test}>
      ...
      <div className="test-detail-modal-root fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 print:bg-white" data-testid="test-detail-modal" onClick={onClose}>
        ...
        <div className="overflow-y-auto p-5 space-y-4" style={{ flex: 1 }}>
          ...
          {/* Data Completeness Thermometer */}
          <div className="flex items-center gap-3 px-1">
            <span className="text-xs font-medium text-slate-500 whitespace-nowrap">{filledFieldCount}/{categoryFields.length} fields</span>
            <div className="flex-1 h-2.5 bg-slate-100 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-700 ${
                  completenessPercent >= 80 ? 'bg-emerald-500' :
                  completenessPercent >= 50 ? 'bg-amber-400' :
                  'bg-slate-300'
                }`}
